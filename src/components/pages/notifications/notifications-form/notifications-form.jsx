      ...
      <Box mt={6} as={Flex} flexWrap={"wrap"} gap={6} color={"text.primary"}>
        ...
        <Box w={"100%"} m={2} as={Flex} justifyContent={"space-between"}>
          <Checkbox
            ...
            isDisabled={disabled}
            defaultChecked={
              !!formData.notification_type.find((choice) => choice === "sms")
            }
            value={!!choices.find((choice) => choice === "sms")}
            onChange={(e) => modifyChoice("sms", e.target.checked)}
          >
            SMS
          </Checkbox>
          <Checkbox
            ...
            isDisabled={disabled}
            defaultChecked={
              !!formData.notification_type.find((choice) => choice === "email")
            }
            value={!!choices.find((choice) => choice === "email")}
            onChange={(e) => modifyChoice("email", e.target.checked)}
          >
            Email
          </Checkbox>
          <Checkbox
            ...
            isDisabled={disabled}
            defaultChecked={
              !!formData.notification_type.find((choice) => choice === "app")
            }
            value={!!choices.find((choice) => choice === "app")}
            onChange={(e) => modifyChoice("app", e.target.checked)}
          >
            App
          </Checkbox>
        </Box>
